...
import os
import json
from CHAMMI.evaluation import evaluate
import warnings
...
def run_benchmark(
    ...
    full_result_df = pd.DataFrame(
        columns=["dataset", "task", "classifier", "accuracy", "f1_score_macro"]
    )

    # Iterrate over each dataset
    for _, row in task_dict.iterrows():
        dataset = row.dataset
    ...
    if classifier == "knn":
        full_result_df.to_csv(
            f"{dest_dir}/{classifier}_{knn_metric}_full_results.csv", index=False
        )
    else:
