...
def main():
    ...
    train_dataset = CellDataset(datadir=args.image_folder, mode="train", mask_flag=True)
    test_dataset = CellDataset(datadir=args.image_folder, mode="test", mask_flag=True)
    ...
    if accelerator.num_processes > 1:
        ...
        if accelerator.is_main_process:
            print(
                f"Gathered {len(all_train_embeddings)} training embeddings from all GPUs"
            )
