...
def custom_collate_fn(batch):
    ...
    valid_batch = [
        (img, row) for img, row in batch if img is not None and row is not None
    ]
...
class UnZippedImageArchive(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image_path = os.path.join(
            self.root_dir,
            plate,
            f"{plate}_{position}_{sample}_{cell_id}_cell_image.png",
        )
...
def extract_features(
    ...
    with torch.no_grad():
        for batch_data in tqdm(
            dataloader,
            desc=f"Extracting features on GPU {accelerator.local_process_index}",
    ...
    if all_features:
        ...
        if accelerator.is_main_process:
            ...
            # Save final results
            torch.save(
                (all_rows_combined, torch.from_numpy(all_features_combined)),
                f"{output_folder}/all_features.pth",
            )

            df = pd.DataFrame(all_rows_combined)
            df.to_csv(f"{output_folder}/metadata.csv", index=False)
            print(
                f"Saved {len(all_rows_combined)} samples with {all_features_combined.shape[1]} features"
            )
...
def main():
    ...
    )  # Choices to get from 'mae', 'subcell', 'vit', 'dinov2', 'dinov3', 'openphenom'
    parser.add_argument(
        "--config_path",
        type=str,
    ...
    dataset = UnZippedImageArchive(root_dir=args.image_folder, transform=transform)
