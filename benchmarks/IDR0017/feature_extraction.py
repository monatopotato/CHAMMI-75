...
def get_features(model: Model, args: dict, dataset: Dataset, out_folder: str) -> None:
    ...
    for patches, _, multi_channel_id, plate_name in tqdm(
        ...
        if current_plate is not None and plate_name != current_plate:
            output_file = os.path.join(
                out_folder, f"{current_plate}_features.safetensors"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--images_folder",
        type=str,
        help="Unzipped plate path. Input to segment in snakemake",
    )
    parser.add_argument(
        "-o",
        "--output_folder",
    ...
    parser.add_argument(
        "--model_type", type=str, help="Tells us what type of model given"
    )
    parser.add_argument(
        "--num_workers",
        type=int,
        help="Tells us how many workers helping in feature extraction",
    )
    parser.add_argument(
        "--batch_size",
        default=4,
        type=int,
        help="Setting the batch size for processing image batch sizes",
    )
