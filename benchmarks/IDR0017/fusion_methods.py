...
def early_fusion(mutation, fusion_config):
    ...
    if not os.path.exists(
        os.path.join(fusion_config["save_dir"], "early_fusion", mutation + ".csv")
    ):
        # Create a dictionary to hold the effect sizes for each reagent
        effect_size_dict = {
            reagent: {"Early Merge": []} for reagent in fusion_config["reagents_list"]
        }
        ...
        mutation_metadata = metadata_df.filter(
            (pl.col("biology.cell_line") == mutation)
            & (pl.col("imaging.channel_type") == "nucleus")
        )

        # Mutation Control Metadata
        mutation_control_metadata = mutation_metadata.filter(
            pl.col("experiment.control") == "negative control"
        )
        print("Fetching control features")
        mutation_control_features = np.array(
            list(
                fetch_embeddings_from_metadata(
                    fusion_config["features_dir"],
                    mutation_control_metadata,
                    model_name=fusion_config["model_name"],
                ).values()
            )
        )
        ...
        for reagent in tqdm(fusion_config["reagents_list"]):
            ...
            reagent_metadata = mutation_metadata.filter(
            ...
            effect_size, img = calculate_effect_size(
                control=mutation_control_features,
        ...
        csv_filename = os.path.join(
            fusion_config["save_dir"], "early_fusion", mutation + ".csv"
        )
        ...
        with open(csv_filename, mode="w", newline="") as csvfile:
            ...
            for reagent in fusion_config["reagents_list"]:
                ...
                row = (
                    [reagent]
                    + [np.mean(effect_size_dict[reagent]["Early Merge"])]
                    + [gt_value]
                )
                writer.writerow(row)
