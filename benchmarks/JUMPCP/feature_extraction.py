...
def configure_dataset(root_dir, plate_name, transform=None):
    ...
    dataset = sc_dataset.SingleCellDataset(root=root_dir, plate=plate_name, metadata_path=metadata_path, transform=transform)
    return dataset
...
def get_save_features(feature_dir, root_dir, model_check, gpu, batch_size, model_path, model_size):
    ...
    for plate in plates_for_fe:
        ...
        feature_path = os.path.join(feature_dir, plate)
