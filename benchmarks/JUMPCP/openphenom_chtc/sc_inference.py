...
def main(dataset_location):
    ...
    lincs_dataset = sc_dataset_openphenom.SingleCellDataset(
        transform=transform, root=dataset_location, metadata_path=metadata_file
    )
    ...
    if channel_wise_embeddings:
        feature_matrix = np.zeros(
            (len(lincs_dataset), 384 * n_channels), dtype=np.float32
        )  # TODO: whole feature matrix should not be stored in RAM
    else:
        feature_matrix = np.zeros(
            (len(lincs_dataset), 384), dtype=np.float32
        )  # TODO: whole feature matrix should not be stored in RAM
    ...
    for samples in tqdm(data_loader):
        ...
        with torch.no_grad():
            ...
            print(latent.shape)
            feature_matrix[batch_size * bi : batch_size * bi + latent.shape[0], :] = (
