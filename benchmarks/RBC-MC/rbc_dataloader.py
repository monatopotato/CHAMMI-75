...
class RBC_Dataloader(Dataset):
    def __init__(self, datadir: str, transform=None):
        self.swiss_image_paths = glob.glob(
            os.path.join(datadir, "Swiss", "**", "*.ome.tif"), recursive=True
        )
        self.canadian_image_paths = glob.glob(
            os.path.join(datadir, "Canadian", "**", "*.ome.tif"), recursive=True
        )

        # Default transform chain
        if transform is None:
            self.transform = lambda img: PerImageNormalize()(
                ToTensorNormalize()(v2.Resize(224)(img))
            )
        else:
