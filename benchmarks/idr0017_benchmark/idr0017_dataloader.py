...
class FeatureExtractionDataset(Dataset):
    def __init__(self, inputs_dir: str, transforms=None):
        ...
        self.plate_names = [entry for entry in os.listdir(inputs_dir) if os.path.isdir(os.path.join(inputs_dir, entry))]
    ...
    def _load_all_coords(self):
        for plate_name in self.plate_names:
            ...
            safetensor_file = [f for f in os.listdir(plate_dir) if f.endswith('.safetensors')]

            if not safetensor_file:
                raise FileNotFoundError(f"No safetensors file found in {plate_dir}")
            
            if len(safetensor_file) > 1:
                raise ValueError(f"Multiple safetensors files found in {plate_dir}, expected only one.")
        
            # Assuming one per plate
            plate_coords = load_file(os.path.join(plate_dir, safetensor_file[0]))

            if not plate_coords:
                raise ValueError(f"Failed to load coordinates from {safetensor_file[0]} in {plate_dir}")
    ...
    def _get_all_image_groups(self):
        ...
        images = self.metadata.select(pl.all().sort_by('imaging.channel').over('imaging.multi_channel_id'))
        ...
        for plate_name in self.plate_names:
            ...
            plate_image_groups = list(plate_metadata.group_by('imaging.multi_channel_id'))
            ...
            for criteria, group in plate_image_groups:
                ...
                for image_name in image_names:
                    ...
                    if st_key in self.coords:
                        ...
                        break
                if found_mask:
                    group_with_plate = group.with_columns(pl.lit(plate_name).alias('plate_name'))
    ...
    def _get_st_key(self, image_name: str, plate_name: str = None) -> str:
        if plate_name is None:
            if '/' in image_name:
                ...
                raise ValueError("Plate name must be provided if image_name does not contain plate info.")
    ...
    def _read_images_get_masks(self, image_names:list, plate_name:str):
        ...
        if segmented_image_name is None:
            ...
            raise ValueError(f"No segmentation coordinates found for any image in {plate_name}. Images: {image_names}")
    ...
    def __getitem__(self, index) -> Tuple[torch.Tensor, list, str]:
        ...
        images, segmented_image_name = self._read_images_get_masks(image_names, plate_name)
