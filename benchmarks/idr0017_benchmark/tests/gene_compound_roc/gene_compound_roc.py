...
import utils
from utils import (
    fetch_embeddings_from_metadata,
...
class GeneCompoundInteraction:
    ...
    def __init__(self, config):
        ...
        if config["mutations"] is not None:
            ...
            self.mutations_list = (
                self.metadata_df["biology.cell_line"].unique().to_list()
            )
            self.config["mutations"] = self.mutations_list

        # Get Unique List of Active Reagents to Analyze
        self.reagents_list = (
            self.metadata_df.filter(pl.col("experiment.control") != "negative control")[
                "experiment.reagent"
            ]
            .unique()
            .to_list()
    ...
    def compute_study_effect_size(self):
        ...
        if self.fusion_type == "early_fusion":
            ...
            with ThreadPoolExecutor(max_workers=self.num_workers) as executor:
                futures = [
                    executor.submit(early_fusion, mutation, fusion_config)
    ...
    def compute_study_auc(self):
        ...
        csv_file_path = os.path.join(
            self.save_dir,
            self.fusion_type,
            f"{self.config['benchmark_test']}_scores.csv",
        )
        ...
        for mutation in self.mutations_list:
            ...
            plot_save_path = os.path.join(
                self.save_dir, self.fusion_type, f"roc_curve_{mutation}.png"
            )

            # Read effect size from csv files
            effect_size_csv = os.path.join(
                self.save_dir, self.fusion_type, f"{mutation}.csv"
            )
            ...
            distance_columns = [
                col
                for col in effect_size_df.columns
                if col not in ["Reagent", "Ground Truth"]
            ]
            ...
            for column in distance_columns:
                ...
                distance_array = (distance_array - np.min(distance_array)) / (
                ...
                if self.config["benchmark_test"] == "auc_roc":
                    x, y, _ = roc_curve(
                        groud_truth_array, distance_array, drop_intermediate=False
                    )
                ...
                # Calculate Recall and Precision AUC score
                    ...
                    precision, recall, thresholds = precision_recall_curve(
                        groud_truth_array, distance_array
                    )
                ...
                # Top-K recall
                elif ("recall" in self.config["benchmark_test"]) and (
                    "_" in self.config["benchmark_test"]
                ):
                ...
                # Raise Exception
                    ...
                    raise ValueError(
                        f"Unknown benchmark test: {self.config['benchmark_test']}"
                    )
    ...
    def compute_replicate_correlation(self):
        ...
        correlation_csv_path = os.path.join(
            self.save_dir, self.fusion_type, "replicate_correlation.csv"
        )
        ...
        for mutation in self.mutations_list:
            ...
            spearman_correlation, _ = spearmanr(
                replicate_1_effect_size, replicate_2_effect_size
            )
    ...
    def late_fusion(self, mutation):
        if not os.path.exists(
            os.path.join(self.save_dir, "late_fusion", mutation + ".csv")
        ):
            # Create a dictionary to hold the effect sizes for each reagent
            effect_sizes_dict = {
                reagent: {self.replicate_list[0]: [], self.replicate_list[1]: []}
            ...
            for replicate in self.replicate_list:
                ...
                replicate_metadata = self.metadata_df.filter(
                    (pl.col("biology.cell_line") == mutation)
                ...
                for plate in unique_plates:
                    ...
                    plate_metadata = replicate_metadata.filter(
                        pl.col("storage.zip") == plate
                    )

                    # Get Control for the plate
                    plate_control_metadata = plate_metadata.filter(
                        (pl.col("experiment.control") == "negative control")
                    )
                    plate_control_features = np.array(
                        list(
                            fetch_embeddings_from_metadata(
                                self.features_dir,
                                plate_control_metadata,
                    ...
                    plate_treated_metadata = plate_metadata.filter(
                        (pl.col("experiment.control") == "no")
                        | (pl.col("experiment.control") == "positive control")
                    )

                    # Get unique reagents in the plate
                    unique_reagents = (
                        plate_treated_metadata["experiment.reagent"].unique().to_list()
                    )
                    ...
                    for reagent in unique_reagents:
                        # Reagents Metadata
                        reagent_metadata = plate_treated_metadata.filter(
                            pl.col("experiment.reagent") == reagent
                        )
                        reagent_features = np.array(
                            list(
                                fetch_embeddings_from_metadata(
                                    self.features_dir,
                                    reagent_metadata,
                        ...
                        effect_size, img = calculate_effect_size(
                            control=plate_control_features,
            ...
            with open(csv_filename, mode="w", newline="") as csvfile:
                ...
                for reagent in effect_sizes_dict:
                    ...
                    if reagent in gt_df["matched_drug_name"].to_list():
                        gt_value = int(
                            gt_df.filter(pl.col("matched_drug_name") == reagent)
                            .select(mutation)
                            .to_numpy()
                            .flatten()[0]
                        )
                    ...
                    mean_effect_size = (
                        np.mean(effect_sizes_dict[reagent][self.replicate_list[0]])
                        + np.mean(effect_sizes_dict[reagent][self.replicate_list[1]])
                    ) / 2
                    row = (
                        [reagent]
                        + [np.mean(effect_sizes_dict[reagent][self.replicate_list[0]])]
