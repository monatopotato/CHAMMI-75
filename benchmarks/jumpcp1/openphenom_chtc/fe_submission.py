...
def generateDAG(prefix, working_dir, max_running):
    ...
    dag.layer(
       ...
       vars = [{"node_name": f"OPFE-{idx}-", "archive": archives[idx]} for idx in range(len(archives))],
       retries = int(1),
    )
    ...
    print("Compress jobs were submitted as DAG with JobID %d.0" % submit_result.cluster())
...
def topMain():
    ...
    parser.add_argument("--instance", help="Instance name for the run", default="op-jump")
    parser.add_argument("-w", "--working-dir", help="Working directory for the DAG associated with the download instance", default=".")
    parser.add_argument("-r", "--max-running", help="Maximum number of running jobs", default=136)
    args = parser.parse_args()
    if countDags(args.instance):
        print(f"Cannot submit new compression named {args.instance}; one already exists in queue")
        return 2
