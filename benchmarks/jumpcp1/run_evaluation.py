...
def run_phenotypic_activity(profiles, model, null_size, batch_size, fdr, output_folder):
    ...
    if model == "cellprofiler":
        brightfield_features = [
            i for i in profiles_activity.columns if "ZBF" in i or "Brightfield" in i
        ]
    ...
    activity_map.to_csv(
        os.path.join(output_folder, model, "phenotypic_activity_all_map.csv")
    )
    active_ratio = activity_map.below_corrected_p.mean()
    mean_map = activity_map[activity_map.below_corrected_p][
        "mean_average_precision"
    ].sum() / len(activity_map)

    activity_map.to_csv(
        os.path.join(output_folder, model, "phenotypic_activity_map.csv")
    )
    pd.DataFrame(
        columns=[
            "Active fraction",
            "mean mAP (all - non-active = 0)",
            "mean mAP (all)",
            "mean mAP (active only)",
        ],
        data=[
            [
                active_ratio,
                ...
                activity_map[activity_map.below_corrected_p][
                    "mean_average_precision"
                ].mean(),
            ]
        ],
...
def run_phenotypic_consistency(
    profiles, activity_map, model, null_size, batch_size, fdr, output_folder
):
    ...
    if model == "cellprofiler":
        brightfield_features = [
            i for i in consensus_profiles.columns if "ZBF" in i or "Brightfield" in i
        ]
    ...
    consensus_profiles = consensus_profiles.groupby(
        ["Metadata_broad_sample"], as_index=False
    )[feature_columns].median()

    total_targets = (
        profiles.merge(utils.read_metadata(), on="Metadata_broad_sample", how="left")
        .assign(
        ...
        set(
            list(
                itertools.chain.from_iterable(
                    total_targets[total_targets.notna()].to_numpy()
                )
            )
        )
    )
    ...
    consistent_map = agg_result[agg_result.below_corrected_p][
        "mean_average_precision"
    ].mean()
    overall_map = (
        agg_result[agg_result.below_corrected_p]["mean_average_precision"].sum()
        / total_targets
    )
    print(
        f"Phenotypic consistency: fraction: {consistent_ratio}, mean mAP for consistent passed targets {agg_result.mean_average_precision.mean()}, Overall mAP where not-passed and non-consistent targets mAP = 0 {overall_map}"
    )

    pd.DataFrame(
        columns=[
            "Consistent fraction",
            "Consistent fraction true",
            "mean mAP for consistent passed targets",
            "mean mAP for passed targets",
            "mean mAP all; targets non-consistent and non-present = 0 ",
        ],
        data=[
            [
                consistent_ratio,
                consistent_true_fraction,
                consistent_map,
                agg_result["mean_average_precision"].mean(),
                overall_map,
            ]
        ],
    ).to_csv(
    ...
    agg_result.to_csv(
        os.path.join(output_folder, model, "phenotypic_consistency_map.csv")
    )
...
def get_parser():
    ...
    parser.add_argument(
        "--feat_dir",
        type=str,
        default="./features/aggregated/",
        help="The directory that contains the aggregated features",
        required=False,
    )
    parser.add_argument(
        "--model",
    ...
    parser.add_argument(
        "--postfix",
        type=str,
        default="group_spherized_0.001",
        help="Postfix for aggregated features name",
        required=False,
    )
    parser.add_argument(
        "--fdr", default=0.05, type=float, help="P-value threshold", required=False
    )
    parser.add_argument(
        "--batch_size",
        default=100000,
        type=int,
        help="Batch size for copairs",
        required=False,
    )
    parser.add_argument(
        "--null_size_pa",
        default=100000,
        type=int,
        help="Null distribution sample size",
        required=False,
    )
...
if __name__ == "__main__":
    ...
    activity_map = run_phenotypic_activity(
        profiles,
        args.model,
        args.null_size_pa,
        args.batch_size,
