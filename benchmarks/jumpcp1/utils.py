...
def read_metadata():
    target1_metadata = (
        ...
        .dropna()
        .assign(col=lambda x: list(x["Metadata_target_list"].str.split("|")))
    )
    ...
    for plate in plates:
        profiles = pd.concat(
            (
                profiles,
                pd.read_csv(
                    os.path.join(feat_dir, model, plate, f"{plate}_{postfix}.csv.gz")
                ),
            )
        )

    profiles = profiles.reset_index(drop=True)
    profiles.loc[profiles.Metadata_pert_iname == "DMSO", "Metadata_broad_sample"] = (
        "DMSO"
    )
    return profiles
