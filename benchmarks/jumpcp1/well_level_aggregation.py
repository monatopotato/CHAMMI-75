...
def aggregate_profiles(plate_filename, profiles_folder, model, feature_size, feature_columns, output_folder):
    ...
    profile_data.to_parquet(os.path.join(output_folder, model, plate, f'{plate}.parquet'))
...
def normalize_features(plates, model, feature_columns, output_folder):
    ...
    for plate in plates:
        ...
        dl_plate = pd.read_parquet(os.path.join(output_folder, model, plate, f"{plate}.parquet"))
        ...
        dl_plate.to_csv(os.path.join(output_folder, model, plate, f'{plate}_raw.csv.gz'), compression='gzip', index = False)
    ...
    for plate in normalized_df.Metadata_Plate.unique():
        ...
        to_save.to_csv(os.path.join(output_folder, model, plate, f'{plate}_group_spherized_0.001.csv.gz'), compression='gzip', index = False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output', help = 'Path to folder to save aggregated profiles for models', required=False)
    ...
    if args.model != 'cp-cnn':
        for plate in tqdm(plates):
            aggregate_profiles(plate, args.profiles, args.model, feature_size, feature_columns, args.output)
