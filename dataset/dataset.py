...
class IterableImageArchive(IterableDataset):
    ...
    def load_archive(self):
        if isinstance(self.config.data_path, list) and len(self.config.data_path) == 2:
            ...
            self.archive = [
                zipfile.ZipFile(path, "r") for path in self.config.data_path
            ]
            self.image_paths = []
            for i, archive in enumerate(self.archive):
                archive_images = [
                    ZipFileWrapper(file, i)
                    for file in archive.infolist()
                    if not file.is_dir()
                    and file.filename.endswith(self.config.img_type)
                ]
            ...
            ]  # and any(ds in file.filename for ds in DS10)]
            print(f"Loaded {len(self.image_paths)} images from {self.config.data_path}")
    ...
    def return_sample(self, file_list: list):
        for file_path in file_list:
            ...
            if isinstance(self.archive, list):
                ...
                zipinfo_obj = (
                    file_path.zipinfo if hasattr(file_path, "zipinfo") else file_path
                )
            ...
            try:
                ...
                if baseline != (-1, -1):
                    ...
                    crop_height = random.randint(
                        int(baseline[0] * 0.9), int(baseline[0] * 1.1)
                    )
                    crop_width = random.randint(
                        int(baseline[1] * 0.9), int(baseline[1] * 1.1)
                    )
                ...
                if self.config.guided_crops_path and self.guided_crops.crop_size != (
                    -1,
                    -1,
                ):
                    safetensors_name = file_path.filename[:-4] + ".safetensors"
                    safetensors_name = safetensors_name.replace(
                        "CHAMMI-75_train", "CHAMMI-75_guidance"
                    )
                    safetensors_name = os.path.join(
                        self.config.guided_crops_path,
                        safetensors_name.split("/", maxsplit=1)[1],
                    )
    ...
    def worker_init_fn(self, worker_id):
        ...
        if dataset.config.guided_crops_path:
            dataset.guided_crops = GuidedCrop(
                dataset.config.guided_crops_size, dataset.config.guided_crops_path
            )
    ...
    def __iter__(self):
        ...
        if self.config.guided_crops_path:
            self.default_transform = v2.RandomResizedCrop(
                size=self.guided_crops.crop_size, antialias=True
            )
    ...
    def __len__(self):
        if not self.image_paths:
            if (
                isinstance(self.config.data_path, list)
                and len(self.config.data_path) == 2
            ):
                # Handle two data paths
                image_paths = []
...
class ChannelViTDataset(IterableImageArchive):
    ...
    def init_channel_map(self, channels: list, save: bool):
        ...
        if save:
            with open(
                os.path.join(self.config.output_dir, "channel_map.json"), "w"
            ) as f:
                f.write(json.dumps(channel_map))
        return channel_map
    ...
    def return_sample(self, file_list: list):
        for file_group, channel_types in file_list:
            ...
            if self.guided_crops:
                ...
                if (
                    self.guided_crops.crop_size != (-1, -1)
                    and self.config.guided_crops_path
                ):
    ...
    def collate_fn(self, samples: list[tuple[torch.Tensor, list[str]]]):
        ...
        for sample in samples:
            ...
            channel_masks.append(
                [True if idx < num_chans else False for idx in range(max_chans)]
            )

        return (
            [self.collate_crops(crops, max_chans) for crops in collate_list],
            channel_list,
            channel_masks,
        )
    ...
    def collate_fn_simclr(self, samples: list):
        max_chans = max(
            [sample[0].shape[0] for sample in samples]
        )  # sample[0] is the tensor
    ...
    def load_dataset_config(self, config_path):
        ...
        if self.config.dataset_filter == "allen":
            self.dataset_config = self.dataset_config.filter(
                pl.col("storage.path").str.contains("/Allen/")
            )
        elif self.config.dataset_filter == "cp":
            self.dataset_config = self.dataset_config.filter(
                pl.col("storage.path").str.contains("/CP/")
            )
        elif self.config.dataset_filter == "hpa":
            self.dataset_config = self.dataset_config.filter(
                pl.col("storage.path").str.contains("/HPA/")
            )
        elif self.config.dataset_filter == "10ds":
            self.dataset_config = self.dataset_config.filter(
                pl.col("experiment.study").is_in(DS10)
            )

        channels = self.dataset_config["imaging.channel_type"].unique().to_list()
        aggregated = (
            self.dataset_config.sort("imaging.channel")
            .group_by("imaging.multi_channel_id", maintain_order=True)
            .agg(pl.col("storage.path"), pl.col("imaging.channel_type"))
        )
        return list(
            zip(
                aggregated["storage.path"].to_list(),
                aggregated["imaging.channel_type"].to_list(),
            )
        ), channels
