...
class GuidedCrop(object):
    def __init__(self, crop_size: tuple[int, int], crop_data: str):
        ...
        self.data_paths = set(
            [str(p) for p in self.crop_data_dir.glob("**/*.safetensors")]
        )
        print(
            f"Found {len(self.data_paths)} guided crop data files in {self.crop_data_dir}"
        )
    ...
    def __call__(self, sample: torch.Tensor, sample_path: str) -> torch.Tensor:
        if sample_path in self.data_paths:
            with Path(sample_path).open("rb") as f:
                ...
                chosen_centroid = possible_centroids[
                    random.randint(0, possible_centroids.shape[0] - 1), :
                ]
                ...
                x1, y1, x2, y2 = get_crop_location(
                    crop_height // 2, crop_width // 2, y, x, image_height, image_width
                )
                cropped_sample = sample[:, y1:y2, x1:x2]
                return cropped_sample
        else:
            raise ValueError(
                "Sample path is not in the guided crop data. Please check why this function was called."
            )
