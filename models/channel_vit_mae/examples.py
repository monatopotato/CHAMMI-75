...
def run_BoC_demo(rank, world_size):
    ...
    with open("simclr_and_mae/model_config.yaml", "r") as f:
        model_cfg = yaml.safe_load(f)

    model_cfg["use_channel_tokens"] = False
...
############ Example 2: Multi-channel ViT SimCLR/MAE
...
if __name__ == "__main__":
    ###### command to run the SimCLR and MAE examples (modify the lambda in `/models/simclr_and_mae/model_config.yaml` to choose which model to run) ######
    ## cd CHAMMI-75/models
    ## torchrun -m simclr_and_mae.examples --nproc_per_node=1

    print(">>>>>> running BoC example ...")
    BoC_main()

    print("\n>>>>>> running multi-channel ViT example ...")
    ...
    $ torchrun -m simclr_and_mae.examples --nproc_per_node=1
    >>>>>> running BoC example ...
    /projectnb/morphem/chaupham/chammiv2/CHAMMI-75/models/simclr_and_mae/vit.py:92: UserWarning: xFormers is available (Attention)
    warnings.warn("xFormers is available (Attention)")
    Output keys: dict_keys(['output', 'proxy_loss', 'supcon_loss', 'simclr_loss', 'mae_img_loss', 'mae_fourier_loss', 'mae_loss', 'loss'])
    Output shape: torch.Size([6, 384])
    Loss: 1.2503890991210938

    >>>>>> running multi-channel ViT example ...
    Output keys: dict_keys(['output', 'proxy_loss', 'supcon_loss', 'simclr_loss', 'mae_img_loss', 'mae_fourier_loss', 'mae_loss', 'loss'])
    Output shape: torch.Size([6, 384])
    Loss: 1.2130433320999146
    """
