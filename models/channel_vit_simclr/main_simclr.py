...
def load_checkpoint(checkpoint_path, model, optimizer, lr_scheduler, device):
    ...
    print(
        f"Resumed from epoch {start_epoch}, global step {global_step}, last loss: {last_loss:.4f}"
    )
...
def check_and_handle_existing_checkpoints(output_dir, args):
    ...
    if latest_checkpoint is None:
        print(
            f"No existing checkpoints found in {output_dir}. Starting fresh training."
        )
        return False, None
    ...
    if hasattr(args, "resume") and args.resume:
        ...
        if args.gpu == 0:
            while True:
                choice = (
                    input(
                        "Do you want to (r)esume from checkpoint, (o)verwrite and start fresh, or (a)bort? [r/o/a]: "
                    )
                    .strip()
                    .lower()
                )
            ...
            if dist.is_initialized():
                ...
                decision_tensor = torch.tensor(
                    [1 if should_resume else 0], dtype=torch.int, device=args.gpu
                )
...
def get_args_parser():
    ...
    parser.add_argument(
        "--warmup_epoch", default=10, type=int, help="number of warmup epochs"
    )
    ...
    parser.add_argument(
        "--batch_size_per_gpu", default=256, type=int, help="Per-GPU batch-size"
    )
    ...
    parser.add_argument(
        ...
        default="small",
        type=str,
        help="Size of the dataset to use: small/medium/full",
    )
    ...
    parser.add_argument(
        ...
        default=False,
        type=bool,
        help="Whether to use guided cropping based on segmentation masks",
    )

    parser.add_argument(
        "--guided_crops_path",
        default="/scr/vidit/chammi_segmentations.zip",
        type=str,
        help="Path to the guided crops segmentation masks zip file",
    )
    ...
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Overwrite existing checkpoints and start fresh training",
    )
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="Filter to select a specific dataset from the metadata",
    )
    return parser
    ...
    def __init__(self, image_size=(224, 224), kernel_size=11):
        ...
        self.augmentation_pipeline = v2.Compose(
            [
                ...
                v2.RandomApply(
                    [v2.GaussianBlur(kernel_size=self.kernel_size, sigma=(0.1, 2.0))],
                    p=0.5,
                ),
            ]
        )
...
def train_simclr(args):
    ...
    should_resume, checkpoint_path = check_and_handle_existing_checkpoints(
    ...
    num_warmup_steps = (
        num_update_steps_per_epoch * args.warmup_epoch
    )  # 10 epochs of warmup
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch_idx, (data, channel_ids_list, channel_masks) in enumerate(
            data_loader
        ):
            # Move data to GPU
            ...
            )  # Duplicate for view1 + view2
            simclr_channel_masks = (
                channel_masks + channel_masks
            )  # Duplicate for view1 + view2

            # Convert to tensors and pad to same length
            max_channels = len(
                simclr_channel_masks[0]
            )  # All masks should have same length

            # Pad channel IDs to max_channels length
            padded_channel_ids = []
            for channel_ids in simclr_channel_ids:
                padded_ids = channel_ids + [0] * (
                    max_channels - len(channel_ids)
                )  # Use -1 as padding
                padded_channel_ids.append(padded_ids)

            # Convert to tensors and move to GPU
            channel_ids_tensor = torch.tensor(
                padded_channel_ids, dtype=torch.long, device=args.gpu
            )
            channel_masks_tensor = torch.tensor(
                simclr_channel_masks, dtype=torch.bool, device=args.gpu
            )
            ...
            if args.gradient_accumulation_steps == 1:
                ...
                if args.gpu == 0 and (global_step % 10 == 0 or global_step < 10):
                    ...
                    if steps_in_epoch > 0:
                        ...
                        eta_epoch = time_per_step * (
                            total_steps_per_epoch - steps_in_epoch
                        )
                        print(
                    ...
                    if args.gpu == 0 and (global_step % 10 == 0 or global_step < 10):
                        ...
                        if global_step > 0:
                            time_per_step = elapsed_time / (
                                global_step - (epoch * total_steps_per_epoch)
                            )
                            eta_epoch = time_per_step * (
                                total_steps_per_epoch
                                - (global_step % total_steps_per_epoch)
                            )
                            print(
                                f"Epoch {epoch + 1}, Step {global_step}/{total_steps}: "
        ...
        if args.gpu == 0:
            ...
            print(
                f"Epoch {epoch + 1} completed in {epoch_duration / 60:.1f} minutes: "
                f"Average Loss = {avg_epoch_loss:.4f}, LR = {current_lr:.6f}"
            )

            if epochs_remaining > 0:
                print(
                    f"ETA for training completion: {eta_total:.1f} minutes ({eta_total / 60:.1f} hours)"
                )
